...
const DEFAULT_CURVE_STEPS = 20;
const DEFAULT_ANIMATION_INTERVAL = 120;
...
export function ConnectionLines({ 
  ...
  const { limitedConnections, connectionPaths } = useMemo(() => {
    ...
    // Prioritize connections by beneficiaries if available
    const sortedConnections = [...validConnections].sort((a, b) => {
      ...
      return beneB - beneA; // Sort by descending beneficiaries count
    });
    
    // Add some randomization to avoid always showing the same connections
    const topHalf = sortedConnections.slice(0, Math.ceil(maxConnections / 2));
    const bottomHalf = sortedConnections.slice(Math.ceil(maxConnections / 2));
    const shuffledBottom = [...bottomHalf].sort(() => Math.random() - 0.5);
    
    // Combine prioritized top connections with some random others
    const selectedConnections = [...topHalf, ...shuffledBottom].slice(0, maxConnections);
